...
def request_json(path, params=None):
    ...
    for i in range(3):
        try:
            ...
            if i == 2: raise RuntimeError(str(e))
            time.sleep(0.6 * (i + 1))
...
def get_klines(symbol, interval="15m", limit=220):
    ...
    return [{"open_time": int(x[0]), "high": to_float(x[2]),
             "low": to_float(x[3]), "close": to_float(x[4])} for x in data]
...
def run_analysis(symbol="BTCUSDT"):
    ...
    lc = (trend=="偏多" and oi_change>0.2 and taker_ratio>1.05 and top_pos_ratio>1.0)
    sc = (trend=="偏空" and oi_change>0.2 and taker_ratio<0.95 and top_pos_ratio<1.0)

    reasons = []
    if lc: reasons.append("仓位增加 + 主动买盘强 + 大户偏多")
    if sc: reasons.append("仓位增加 + 主动卖盘强 + 大户偏空")
    if trend=="偏多" and oi_change<-0.2: reasons.append("价格走强但仓位下降，像空头回补")
    if trend=="偏空" and oi_change<-0.2: reasons.append("价格走弱但仓位下降，像多头出清")
    if cl: reasons.append("资金费率偏正，多头拥挤")
    if cs: reasons.append("资金费率偏负，空头拥挤")
    ...
    if state=="震荡整理": action="观望"; conf="中性"; reasons.append("当前震荡，不适合追单")
    elif state=="缩量等待": action="观望"; conf="中性"; reasons.append("波动收缩，等待放量")
    else:
        if lc and not cl: action="建议做多"; conf="强"
        elif sc and not cs: action="建议做空"; conf="强"
        elif lc and cl: action="偏多但别追"; conf="中"
        elif sc and cs: action="偏空但别追"; conf="中"
        elif trend=="偏多": action="偏多等确认"; conf="弱"
        elif trend=="偏空": action="偏空等确认"; conf="弱"
        else: action="观望"; conf="弱"

    entry = close
    if "做多" in action or action.startswith("偏多"):
        sl=support-atr14*0.5; risk=abs(entry-sl)
        tp1=entry+risk*1.5; tp2=entry+risk*2.5
    elif "做空" in action or action.startswith("偏空"):
        sl=resistance+atr14*0.5; risk=abs(sl-entry)
        tp1=entry-risk*1.5; tp2=entry-risk*2.5
    else:
        if trend=="偏多":
            sl=support-atr14*0.5; risk=abs(entry-sl)
    ...
    return {
        ...
        "reasons":reasons,"entry":entry,"stop_loss":sl,
        "take_profit_1":tp1,"take_profit_2":tp2,
        "rr":safe_div(abs(tp2-entry),abs(sl-entry)),
        "close":close,"trend":trend,"state":state,
        "rsi":rsi14,"atr":atr14,"support":support,"resistance":resistance,
        "oi_change":oi_change,"taker_ratio":taker_ratio,
        "top_pos_ratio":top_pos_ratio,"global_ratio":global_ratio,
        "last_fr":last_fr,"funding_state":fstate,
        "chart":[x["close"] for x in kl[-60:]],
...
def get_all_symbols():
    ...
    now = time.time()
    if _symbols_cache and now - _symbols_ts < 3600: return _symbols_cache
    data = request_json("/fapi/v1/exchangeInfo")
    syms = [{"symbol":s["symbol"],"baseAsset":s.get("baseAsset","")}
            for s in data.get("symbols",[])
            if s.get("status")=="TRADING" and s.get("contractType")=="PERPETUAL"]
    syms.sort(key=lambda x: x["symbol"])
    _symbols_cache = syms; _symbols_ts = now
    return syms
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/symbols":
            ...
            self.end_headers()
            import os
            html_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "widget_ui.html")
...
def main():
    ...
    try:
        ...
        try:
            ...
            def install():
                import subprocess
                subprocess.run(["pip", "install", "pywebview", "-q"])
                messagebox.showinfo("完成", "安装完成！请重新双击运行 widget.py")
                root.destroy()
            btn = tk.Button(root, text="自动安装 pywebview", command=install,
                bg="#1f6feb", fg="white", font=("Microsoft YaHei", 11),
