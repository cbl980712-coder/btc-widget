...
def _get_structure_zones(highs, lows, atr, interval):
    ...
    # min_zone_pct：主流0.5%，中等1.0%，小币1.5%
    if price >= 500:
        min_zone_pct = 0.005
    elif price >= 1:
        min_zone_pct = 0.010
    ...
    zone_width = max(0.5 * atr, price * min_zone_pct)

    # 多单 support_zone = [structure_low, structure_low + zone_width]
    # 空单 resistance_zone = [structure_high - zone_width, structure_high]
    support_zone    = [smart_round(structure_low),
                       smart_round(structure_low + zone_width)]
    resistance_zone = [smart_round(structure_high - zone_width),
                       smart_round(structure_high)]
...
IS_OKX = False  # OKX版为True，用于放宽容错参数（由各自入口设置）
...
# 实际使用时根据 price 段动态选取
MIN_SL_PCT = {
    # 各周期最小止损距离，小于此值降级观察（不进A/B）
    # high=主流币（BTC/ETH，价格>=500），mid=中等币（价格1~500），low=小币/meme（价格<1）
    "5m":  {"high": 0.008, "mid": 0.012, "low": 0.020},  # scalp
    "15m": {"high": 0.010, "mid": 0.018, "low": 0.025},  # trade，主流1%起，小币2.5%起
    "1h":  {"high": 0.012, "mid": 0.020, "low": 0.030},
    "4h":  {"high": 0.015, "mid": 0.025, "low": 0.035},
    "1d":  {"high": 0.018, "mid": 0.030, "low": 0.040},
}

def get_min_sl_pct(interval, price):
    """根据周期和价格段动态返回最小止损百分比
    price >= 500  → high（主流币：BTC/ETH/BNB等）
    price 1~500   → mid（中等币）
    price < 1     → low（小币/meme）
    """
...
def _calc_entry_model(trend, entry, atr, interval, signal_grade, higher_trend,
    ...
    if trend == "多":
        ...
        if support_zone:
            ...
            # entry = zone 中点（不跟 markPrice）
            entry_price_suggested = smart_round(support_zone[0] + (support_zone[1] - support_zone[0]) * 0.5)
            entry_zone_basis = "support_zone"
        else:
            entry_zone = [smart_round(structure_low * 1.001 if structure_low else entry * 0.985),
                          smart_round(structure_low * 1.008 if structure_low else entry * 0.99)]
            entry_price_suggested = smart_round(structure_low * 1.005 if structure_low else entry * 0.985)
            entry_zone_basis = "atr_pullback"
        # invalid_price = structure_low 下方（不靠近 entry_zone）
        # invalid_price = structure_low 外0.5%（含0.3%容错缓冲，wick触碰不算失效）
        entry_zone_invalid_price = smart_round(structure_low * 0.995) if structure_low else smart_round(entry_zone[0] - atr * 0.8)
        ...
        if resistance_zone:
            ...
            # entry = zone 中点
            entry_price_suggested = smart_round(resistance_zone[1] - (resistance_zone[1] - resistance_zone[0]) * 0.5)
            entry_zone_basis = "resistance_zone"
        else:
            entry_zone = [smart_round(structure_high * 0.992 if structure_high else entry * 1.01),
                          smart_round(structure_high * 0.999 if structure_high else entry * 1.015)]
            entry_price_suggested = smart_round(structure_high * 0.995 if structure_high else entry * 1.015)
            entry_zone_basis = "atr_pullback"
        # invalid_price = structure_high 外0.5%（含0.3%容错缓冲，wick触碰不算失效）
        entry_zone_invalid_price = smart_round(structure_high * 1.005) if structure_high else smart_round(entry_zone[1] + atr * 0.8)
    ...
    # 30分钟绝对保护（详情页）
    _sk2 = (symbol, interval, trend, signal_grade)
    _se2 = _dur_tracker.get(_sk2)
    _al2 = int(time.time()) - _se2["first_seen"] if _se2 else 0
    if _al2 < 1800:  # < 30分钟，强制不失效
        stale_signal = False
...
def _sim_check_pending(symbol, market_price):
    ...
    当 market_price 进入 entry_zone 时才真正开仓。
    每次 tick 时 expire_bars -= 1，到 0 自动取消。
    返回已触发成交的 order 列表。
    """
    triggered = []
    with _sim_pending_lock:
        ...
        remaining = []
        for order in orders:
            order["expire_bars"] = order.get("expire_bars", 5) - 1
            if order["expire_bars"] <= 0:
                order["status"] = "expired"
                continue  # 过期取消
            ez = order.get("entry_zone")
            if ez and ez[0] <= market_price <= ez[1]:
...
def _sim_liquidation_price(trend, entry, leverage, mmr=0.005):
    """爆仓价（含维持保证金率 MMR，默认0.5%，与币安逐仓模式接近）
    ...
    if leverage <= 0:
        return None
    if trend == "多":
        return smart_round(entry * (1 - 1 / leverage + mmr))
    else:
        return smart_round(entry * (1 + 1 / leverage - mmr))
...
def _sim_open_position(symbol, interval, trend, entry_type,
    ...
    with _sim_account_lock:
        ...
        fill_price = smart_round(market_price + slip if trend == "多" else market_price - slip)  # 多单买入价更高，空单卖出价更低
...
def _sim_close_partial(pos, qty_to_close, close_price, reason, fee_rate, slippage_rate):
    ...
    滑点方向：多单平仓(卖出)→成交价更低；空单平仓(买入)→成交价更高
    """
    ...
    # 累计平仓手续费（供统计展示）
    pos["close_fee"] = round(pos.get("close_fee", 0.0) + fee, 4)
    return net, fill
...
def get_sim_account():
    ...
    with _sim_account_lock:
        ...
        total_fee = sum(p.get("open_fee",0) + p.get("close_fee",0) for p in cls+ops)
        ...
        return {
            ...
            "today_pnl":         round(_sim_account["today_pnl"], 4),            # 仅已实现盈亏，不含浮盈（对齐交易所）
...
def get_overview(interval="15m", top_n=50):
    ...
    if _sim_account["config"].get("enabled"):
        ...
        for r in tradeable:
            ...
            # tick 已有仓位（修复：拉真实K线高低价，避免止损穿透）
            if mp_r:
                try:
                    _kl_tick = binance_get("/fapi/v1/klines",
                                           {"symbol": sym_r, "interval": interval, "limit": "2"}, timeout=3)
                    _tick_high = float(_kl_tick[-1][2])
                    _tick_low  = float(_kl_tick[-1][3])
                except Exception:
                    _tick_high = mp_r * 1.002   # fallback: ±0.2%
                    _tick_low  = mp_r * 0.998
